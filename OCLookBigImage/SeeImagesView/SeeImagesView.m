...
- (void) setObj:(id)aa ImageArray:(NSMutableArray *)array
{
    ...
    self.MyImageView.image = [UIImage imageNamed:self.imgOb.name];
...
-(void)singleTap:(UITapGestureRecognizer*)recognizer{//参数轻敲类型的
    ...
    for (int i = 0; i< self.arr_Imgs.count; i++) {
        ...
        imgv.image = [UIImage imageNamed:jj.name];
...
- (UIView *) topView
{
    if (!_topView) {
        ...
        //宽
        [_topView addConstraint:[NSLayoutConstraint constraintWithItem:btn_back attribute:NSLayoutAttributeWidth relatedBy:NSLayoutRelationEqual toItem:nil attribute:NSLayoutAttributeNotAnAttribute multiplier:1.0f constant:20]];
        
        //高
        [_topView addConstraint:[NSLayoutConstraint constraintWithItem:btn_back attribute:NSLayoutAttributeHeight relatedBy:NSLayoutRelationEqual toItem:nil attribute:NSLayoutAttributeNotAnAttribute multiplier:1.0f constant:20]];
